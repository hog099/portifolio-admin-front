...
export const DialogConfirm: React.FC<Props> = ({ isOpen = false, loading = false, title, onConfirm = () => null, onClose = () => null }) => {
  useEffect(() => {
  ...
  return (
    <div className={`${isOpen ? "block" : "hidden"} fixed top-0 left-0 right-0 z-40 w-screen h-screen flex flex-col items-center bg-black/[.1]`} onClick={onClose}>
      <div className={`w-[90%] h-[600px] md:w-[500px] md:h-[200px] mt-[30vh] bg-white shadow-md rounded-md`}>
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700" onClick={(e:any)=>e.stopPropagation()}>
          <button
            type="button"
